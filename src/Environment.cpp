...
{
    using InsertingFunc = std::function<void(Environment& , std::string , std::string)>;

    InsertingFunc inserter;

    if(isParentPathSet())
        inserter = &Environment::addRelativeIconPath;
    else inserter = &Environment::addIconPath;
...
void Environment::addRelativeIconPath(std::string relative_path, std::string object_id)
{
    frontend_icons_[object_id] = parent_path_ / std::filesystem::path{relative_path} ;
}
...
void Environment::addIconPath(std::string path, std::string object_id)
{
    ...
    else addNormalIconPath(path,object_id);
}
