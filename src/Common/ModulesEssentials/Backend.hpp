...
public:
  String(const std::string &str) : body_{str} {}
  String(const QString &str) : body_{str.toStdString()} {}
  String(const char *str) : body_{str} {}
  operator QString() const { return QString::fromStdString(body_); }
  operator std::string() { return body_; }
  operator const char *() const { return body_.c_str(); }
