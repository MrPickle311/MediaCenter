...
struct BackendMediatorsMocks {
  ...
  BackendMediatorsMocks()
      : data_storage_{new MediatorMock},
        environment_{new MediatorMock}, settings_{new MediatorMock} {}
};

class BackendTEST : public MediatorTester {
public:
  BackendTEST()
      : MediatorTester{"test_data/QueryAboutData.json",
                       "test_data/RequestActionData.json"} {
...
protected:
  ...
  // special cases
  void simpleMultimediaEngineTest(std::string call_data,
                                  std::string precall_data,
                                  int calls_count = 1) {
    auto wrapper{createQueryAboutCaller(*mocks_.data_storage_,
                                        factories_.loadQueryPackage(call_data),
                                        calls_count)};
    wrapper->setPrecall(createQueryAboutCaller(
        *mocks_.settings_, factories_.loadQueryPackage(precall_data),
        calls_count));
...
struct MultimediaEngineMocks {
  ...
  MultimediaEngineMocks()
      : audio_{new ProxyMock}, video_{new ProxyMock}, images_{new ProxyMock},
...
public:
  MultimediaEngineTests()
    ...
    builder.at("Audio").addBinding("SearchAudio").addBinding("PlaylistAudio");

    builder.at("Video").addBinding("SearchVideo");

    builder.at("Images").addBinding("SearchImages");
...
public:
  MultimediaEngineIntegrationTests()
    ...
    conf.from("Backend").to("MultimediaEngine").addBinding("SearchAudio");

    conf.from("MultimediaEngine").to("AudioEngine").addBinding("SearchAudio");

    conf.from("AudioEngine")
        .to("MultimediaEngine")
        .addBinding("MediapathsAudio");

    conf.from("MultimediaEngine").to("Backend").addBinding("MediapathsAudio");
  }
};
