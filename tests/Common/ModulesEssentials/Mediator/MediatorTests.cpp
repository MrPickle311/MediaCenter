...
TEST_F(BackendTEST, UnsupportedCommand)
{
    ...
    auto wrapper{createQueryAboutCaller(
        *mocks_.data_storage_, pack, expected_calls_count)};
...
TEST_F(BackendTEST, MultipleCall)
{
    ...
    simpleMultimediaEngineTest(
        "ImagesSearch1", "MediaPathsImages1", calls_count);
}
...
TEST_F(BackendTEST, MixedCalls)
{
    ...
    QueryAboutPackage multimedia_engine_pack{
        factories_.loadQueryPackage("AudioPlaylist1")};

    QueryAboutPackage settings_pack{
        factories_.loadQueryPackage("MediaPathsAudio1")};

    auto wrapper{createQueryAboutCaller(
        *mocks_.data_storage_, multimedia_engine_pack, calls_count)};
    wrapper->setPrecall(
        createQueryAboutCaller(*mocks_.settings_, settings_pack, calls_count));
...
TEST_F(MultimediaEngineTests, MultipleCall)
{
    ...
    auto audio_backend_precall_pack{
        factories_.loadQueryPackage("MediaPathsAudio1")};

    auto audio_wrapper{
        createQueryAboutCaller(*mocks_.audio_, audio_pack, calls_count)};
    audio_wrapper->setPrecall(createQueryAboutCaller(
        *mocks_.backend_, audio_backend_precall_pack, calls_count));
    ...
    auto video_backend_precall_pack{
        factories_.loadQueryPackage("MediaPathsVideo1")};

    auto video_wrapper{
        createQueryAboutCaller(*mocks_.video_, video_pack, calls_count)};
    video_wrapper->setPrecall(createQueryAboutCaller(
        *mocks_.backend_, video_backend_precall_pack, calls_count));
